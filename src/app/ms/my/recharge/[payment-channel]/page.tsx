...
export default function PaymentChannelPage() {
    ...
    const selectedCoin = coins.find(
        coin => coin.label.toLowerCase() === String(paymentChannel).toLowerCase()
    );
    ...
    const transferNetwork =
        typeof paymentChannel === 'string' && paymentChannel.includes('.')
            ? paymentChannel.split('.')[1]
            : paymentChannel;
    ...
    if (!amount || isNaN(Number(amount)) || Number(amount) < 1) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen w-full">
                <div className="text-red-500 text-lg font-semibold">Invalid or missing amount.</div>
            </div>
        );
    }
        ...
        <div className="min-h-screen w-full flex flex-col items-center justify-start bg-gradient-to-b from-orange-200 to-white relative">
            ...
            <div className="w-full max-w-md flex flex-col items-center">
                <div className="bg-gray-100 rounded-md px-6 py-4 w-full flex flex-col items-start space-y-4">
                    ...
                    <div className="mb-2 text-sm text-gray-500 tracking-wider">
                        <span className="font-medium">Deposit amount</span>: <span className="font-bold text-gray-700">{amount}</span>
                    </div>
