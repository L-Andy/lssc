            ...
            <div className="w-full max-w-3/4 flex flex-col items-center">
                ...
                <div className="flex flex-col items-center w-full">
                    <div className="flex flex-row items-center justify-center bg-white/40 rounded-2xl py-4 px-4 mb-10" style={{ width: 350 }}>
                        <button className="w-14 h-14 flex items-center cursor-pointer justify-center mr-6 bg-transparent" onClick={() => setAmount((a) => Math.max(1, a - 1))}>
                        ...
                        <span className="text-xl font-bold text-gray-800 mx-4 min-w-[80px] text-center">{amount}</span>
                        <button className="w-14 h-14 flex items-center cursor-pointer justify-center ml-6 bg-transparent" onClick={() => setAmount((a) => a + 1)}>
                            <svg width="56" height="56" viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ background: 'transparent' }}>
                                <circle cx="28" cy="28" r="26" stroke="#444" strokeWidth="3" fill="none" />
                                <rect x="16" y="27" width="24" height="2" rx="1" fill="#222" />
                    ...
                    <button className="w-[300px] py-3 rounded-lg bg-white/40 cursor-pointer text-lg tracking-wider text-gray-800 font-medium shadow hover:bg-gray-200 transition" onClick={handleConfirm}>
                        Confirm
                    </button>
                </div>
            </div>
...
function RechargeOption({ icon, label, selected, onClick }: { icon: string; label: string; selected?: boolean; onClick?: () => void }) {
    return (
        <div className={`flex flex-col items-center justify-center bg-gray-300 rounded-2xl py-8 shadow cursor-pointer hover:bg-gray-200 transition border-4 ${selected ? 'border-blue-400' : 'border-transparent'}`} onClick={onClick}>
